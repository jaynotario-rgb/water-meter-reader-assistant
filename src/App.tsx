...
type HistoryStatusFilter = 'all' | 'pending' | 'paid' | 'void';

function sameLocalDate(iso: string, date: string) {
  return new Date(iso).toLocaleDateString('en-CA') === date;
}

function isActive(record: ReadingRecord) {
  return (record.recordState ?? 'ACTIVE') === 'ACTIVE';
}

function matchesHistoryStatus(record: ReadingRecord, filter: HistoryStatusFilter) {
  if (filter === 'all') return true;
  if (filter === 'void') return !isActive(record);
  if (!isActive(record)) return false;
  if (filter === 'paid') return (record.paymentStatus ?? 'UNPAID') === 'PAID';
  return (record.paymentStatus ?? 'UNPAID') === 'UNPAID';
}
...
function receiptText(record: ReadingRecord) {
  return [
    ...
    ...(record.editedAt ? [`Edited: ${new Date(record.editedAt).toLocaleString()}`, `Edit Reason: ${record.editReason ?? ''}`] : []),
    ...(record.voidedAt ? [`VOIDED: ${new Date(record.voidedAt).toLocaleString()}`, `Void Reason: ${record.voidReason ?? ''}`] : []),
    '',
...
export function App() {
  ...
  async function lookupMeter(raw = meterNumber) {
    ...
    if (customerRecords[0]) setPreviousReading(String(customerRecords[0].currentReading));
    setMessage(`Existing meter found. Latest active reading loaded for ${customer.name}.`);
  }
  ...
  async function saveReading(event: FormEvent) {
    ...
    try {
      ...
      const record: ReadingRecord = {
        ...
        total: result.total, status: result.status, paymentStatus: 'UNPAID', recordState: 'ACTIVE', capturedAt: now,
  ...
  async function markUnpaid(record: ReadingRecord) {
    if (!isActive(record) || !window.confirm('Mark this record UNPAID again? The reading itself will not be changed.')) return;
    await db.records.update(record.id, {
  ...
  async function editRecord(record: ReadingRecord) {
    if (!isActive(record)) return;
    if (!window.confirm('You are about to EDIT a saved field record. The original values will be preserved in its audit history. Continue?')) return;
    const reason = window.prompt('Reason for editing this record (required)')?.trim();
    if (!reason) { setMessage('Edit cancelled. A reason is required.'); return; }
    const previousText = window.prompt('Previous Reading', String(record.previousReading));
    if (previousText === null) return;
    const currentText = window.prompt('Current Reading', String(record.currentReading));
    if (currentText === null) return;
    const nextPrevious = Number(previousText);
    const nextCurrent = Number(currentText);
    if (!Number.isFinite(nextPrevious) || !Number.isFinite(nextCurrent)) {
      setMessage('Edit cancelled. Readings must be valid numbers.');
      return;
    }
    const next = calculateBilling(nextPrevious, nextCurrent);
    const revision: RecordRevision = {
      editedAt: new Date().toISOString(), reason,
      previousReading: record.previousReading, currentReading: record.currentReading,
      consumption: record.consumption, excessConsumption: record.excessConsumption,
      minimumCharge: record.minimumCharge, excessCharge: record.excessCharge,
      total: record.total, status: record.status,
    };
    if (!window.confirm(`Confirm edit?\n\nPrevious: ${record.previousReading} → ${nextPrevious}\nCurrent: ${record.currentReading} → ${nextCurrent}\nTotal: ${money.format(record.total)} → ${money.format(next.total)}\n\nThis record will be marked EDITED.`)) return;
    await db.records.update(record.id, {
      previousReading: next.previousReading,
      currentReading: next.currentReading,
      consumption: next.consumption,
      excessConsumption: next.excessConsumption,
      minimumCharge: next.minimumCharge,
      excessCharge: next.excessCharge,
      total: next.total,
      status: next.status,
    ...
    await refreshRecords();
    setMessage('Record updated and marked EDITED. Original values remain in the audit history.');
  }

  async function voidRecord(record: ReadingRecord) {
    if (!isActive(record)) return;
    const paidWarning = (record.paymentStatus ?? 'UNPAID') === 'PAID' ? '\n\nWARNING: This record is marked PAID.' : '';
    if (!window.confirm(`VOID this saved record? It will be excluded from active totals but preserved as audit evidence.${paidWarning}`)) return;
    const reason = window.prompt('Reason for voiding this record (required)')?.trim();
    if (!reason) { setMessage('Void cancelled. A reason is required.'); return; }
    if (!window.confirm('Final confirmation: mark this record VOID? This action is intentionally not a hard delete.')) return;
    await db.records.update(record.id, {
      recordState: 'VOID',
    ...
    await refreshRecords();
    setMessage('Record marked VOID and removed from active totals. Audit evidence was preserved.');
  }
  ...
  const filteredFolderRecords = folderRecords.filter((r) => {
    if (!matchesHistoryStatus(r, historyStatus)) return false;
  ...
  async function saveReceipt(record: ReadingRecord) {
    try {
      ...
      setMessage('Receipt saved as a PNG image on this device.');
    } catch (error) {
      console.error(error); setMessage('Could not create the receipt image.');
    }
  }

  async function shareReceipt(record: ReadingRecord) {
    if (!online) { setMessage('Offline: save the receipt now and share it later when connected.'); return; }
    try {
      const file = await receiptPng(record);
  ...
  return <main className="app-shell">
    ...
    {screen === 'reading' && <form className="reading-card" onSubmit={saveReading}>
      ...
      <div className="reading-grid"><label>Previous<input className="number-input" type="number" inputMode="decimal" step="any" value={previousReading} onChange={(e) => setPreviousReading(e.target.value)} placeholder="120" /></label><label>Current<input className="number-input" type="number" inputMode="decimal" step="any" value={currentReading} onChange={(e) => setCurrentReading(e.target.value)} placeholder="145" /></label></div>
      {result && <section className="calculation">{result.status === 'FLAGGED' && <div className="warning"><strong>Reading requires verification.</strong><span>Current reading is lower than previous reading.</span></div>}<div><span>Consumption</span><strong>{result.consumption} m³</strong></div><div><span>Minimum ({result.minimumConsumption} m³)</span><strong>{money.format(result.minimumCharge)}</strong></div><div><span>Excess ({result.excessConsumption} m³ × ₱2.00)</span><strong>{money.format(result.excessCharge)}</strong></div><div className="total"><span>TOTAL</span><strong>{money.format(result.total)}</strong></div></section>}
    ...
    {screen === 'folder' && <section className="screen-card"><button className="back-button" type="button" onClick={() => setScreen('history')}>← BACK TO HISTORY</button><div className="section-heading"><div><p className="eyebrow">CUSTOMER FOLDER</p><h2>{folderCustomer?.customerName ?? 'Customer'}</h2><span className="meter-emphasis">{folderCustomer?.meterNumber}</span><span>History status: {historyStatus === 'all' ? 'All Records' : historyStatus.toUpperCase()}</span></div><strong>{filteredFolderRecords.length}</strong></div>
      <div className="filter-bar"><button className={folderFilter === 'all' ? 'active' : ''} type="button" onClick={() => setFolderFilter('all')}>ALL DATES</button><button className={folderFilter === 'month' ? 'active' : ''} type="button" onClick={() => setFolderFilter('month')}>MONTH</button><button className={folderFilter === 'date' ? 'active' : ''} type="button" onClick={() => setFolderFilter('date')}>DATE</button></div>
      ...
      {filteredFolderRecords.length === 0 ? <p className="empty-state">No readings for the selected status and period.</p> : filteredFolderRecords.map((r) => <article className={`record-row ${!isActive(r) ? 'record-void' : ''}`} key={r.id}><div><strong>{new Date(r.capturedAt).toLocaleDateString()}</strong><span>{new Date(r.capturedAt).toLocaleTimeString()} · {r.id.slice(0, 8).toUpperCase()}</span>{r.editedAt && <span className="audit-edited">EDITED · {new Date(r.editedAt).toLocaleString()}</span>}{r.voidedAt && <span className="audit-void">VOID · {new Date(r.voidedAt).toLocaleString()}</span>}{r.paidAt && <span>Paid: {new Date(r.paidAt).toLocaleString()}</span>}</div><div className="record-amount"><strong>{money.format(r.total)}</strong>{!isActive(r) ? <span className="status-void">VOID</span> : <span className={(r.paymentStatus ?? 'UNPAID') === 'PAID' ? 'status-paid' : 'status-unpaid'}>{(r.paymentStatus ?? 'UNPAID') === 'UNPAID' ? 'PENDING' : 'PAID'}</span>}</div><dl><div><dt>Previous</dt><dd>{r.previousReading}</dd></div><div><dt>Current</dt><dd>{r.currentReading}</dd></div><div><dt>Consumption</dt><dd>{r.consumption} m³</dd></div></dl>{r.editReason && <p className="audit-reason">Edit reason: {r.editReason}</p>}{r.voidReason && <p className="audit-reason">Void reason: {r.voidReason}</p>}<div className="record-actions"><button type="button" onClick={() => openReceipt(r.id)}>RECEIPT</button>{isActive(r) && <button type="button" onClick={() => void editRecord(r)}>EDIT</button>}{isActive(r) && ((r.paymentStatus ?? 'UNPAID') === 'UNPAID' ? <button type="button" onClick={() => void markPaid(r)}>MARK PAID</button> : <button type="button" onClick={() => void markUnpaid(r)}>MARK PENDING</button>)}{isActive(r) && <button className="danger-button" type="button" onClick={() => void voidRecord(r)}>VOID RECORD</button>}</div></article>)}
    </section>}

    {screen === 'receipt' && selectedRecord && <section className="screen-card receipt-shell"><button className="back-button no-print" type="button" onClick={() => selectedCustomerId ? setScreen('folder') : setScreen('history')}>← BACK</button><section ref={receiptRef} className="receipt-card"><div className="receipt-header"><p className="eyebrow">FIELD RECEIPT</p><h2>Water Meter Reader Assistant</h2><span>Record {selectedRecord.id.slice(0, 8).toUpperCase()}</span>{!isActive(selectedRecord) ? <strong className="receipt-status void">VOID</strong> : <strong className={`receipt-status ${(selectedRecord.paymentStatus ?? 'UNPAID').toLowerCase()}`}>{selectedRecord.paymentStatus ?? 'UNPAID'}</strong>}</div>{selectedRecord.status === 'FLAGGED' && <div className="warning"><strong>READING REQUIRES VERIFICATION</strong></div>}{selectedRecord.editedAt && <div className="audit-panel"><strong>EDITED RECORD</strong><span>{new Date(selectedRecord.editedAt).toLocaleString()}</span><span>{selectedRecord.editReason}</span><small>{selectedRecord.revisions?.length ?? 1} revision(s) preserved</small></div>}{selectedRecord.voidedAt && <div className="void-panel"><strong>VOID RECORD</strong><span>{new Date(selectedRecord.voidedAt).toLocaleString()}</span><span>{selectedRecord.voidReason}</span></div>}
